...
public class ResponseEntityTestController {
    ...
    public ResponseEntity<ResponseMessage> findAllUsers() {
        ...
        ResponseMessage responseMessage = new ResponseMessage(200, "성공", responseMap);
        return new ResponseEntity<>(responseMessage, headers, HttpStatus.OK);
    }

    /* 설명. 2. 빌더를 활용하한 메소드 체이닝 방식으로 ResponseEntity 객체 만들기(트랜드) */
     ...
     *  객체(UserDTO)를 활용해야 하며 커맨드 객체는 json 문자열의 프로퍼티 명과 일치해야  한다.
     *
     * 설명.
     *  {
     *      "id" : "user01"
     *  }
     *  이 때 "id" : "user01"을 프로퍼티, '프로퍼티명'이라고 한다.
     * */
    @PostMapping("/users")
