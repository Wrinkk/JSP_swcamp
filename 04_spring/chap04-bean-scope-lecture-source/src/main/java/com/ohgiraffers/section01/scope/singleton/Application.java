...
public class Application {
    ...
    /* 설명.
     *  bean scope란?
     *   스프링 빈이 생설될 때 해당 인스턴스의 범위를 의미한다. 스프링에서는 다양한 bean scope를 제공한다.
     *   1. singleton: 하나의 인스턴스만 생성하고, 모든 빈이 해당 인스턴스를 공유하여 사용한다.
     *   2. prototype: 매번 새로운 인스턴스를 생성한다.
     *   3. request: Http 요청을 처리할 떄마다 새로운 인스턴스를 생성하고, 요청 처리가 끝나면 인스턴스를 폐기한다.
     *              웹 애플리케이션 컨텍스트에만 해당된다.
     *   4. session: Http 세션 당 하나의 인스턴스르 생성하고, 세션이 종료되면 인스턴스를 폐기한다.
     *              웹 애플리케이션 컨텍스트에만 해당된다.
