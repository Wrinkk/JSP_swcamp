...
//        System.out.println("After Advice 동작");
    ...
    /* 설명. Pointcut에 해당하는 위치가 담긴 메소드가 같은 클래스에 있으면 클래스 소속을 안적어도 된다. */
    /* 설명. returning에 쓰인 이름이 반환값이 넘어오는 매개변수명과 일치해야 한다.(result)*/
    @AfterReturning(pointcut = "logPointcut()", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        System.out.println("After Returning result: " + result);

        /* 설명. AfterReturning Adivce를 활용한 반환값 가공도 가능하다. */
     ...
     *  심지어 원본 조인 포인트를 언제 싱행하지, 실행 자체를 안할지, 계속 실행할지 여부까지도 제어한다.
